...
PROMPT_FILE="$OUT_DIR/lmstudio_repo_review_prompt_$(date -u +%Y%m%dT%H%M%SZ).md"

{
  echo "# Local LM Studio second-opinion review"
  echo
  echo "You are reviewing a weather ML / GitHub Pages codebase."
  echo
  echo "Focus on correctness, not style."
  echo
  echo "Return concise, actionable findings. Do not rewrite the entire diff."
  echo
  echo "Review for:"

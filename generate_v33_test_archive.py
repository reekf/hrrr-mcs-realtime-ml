...
forecasts, their ensemble mean, WPC ERO, and the viewer's Practically Perfect
Any flood-proxy verification.
"""
...
def available_dates() -> list[str]:
    paths = [prediction_path(radius) for radius in RADII]
    date_sets = []
    for path in paths:
...
def write_status(date: str, destination: Path) -> None:
    ...
    status = {
        ...
        "product_description": "Machine-learning radius products, ensemble mean, WPC ERO, and Practically Perfect verification.",
        "map_available": True,
        "map_data": "map.json",

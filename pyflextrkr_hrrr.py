...
def _case_signature(
    ...
    *,
    ...
    return {
        ...
        "time_resolution_hours": float(time_resolution_hours),
        "ir_field_name": str(ir_field_name),
...
def _config(
    ...
    *,
    ...
    return {
        ...
        "datatimeresolution": float(time_resolution_hours),
        "tb_varname": "tb",
        ...
        "timegap": float(time_resolution_hours) + 0.1,
        "nmaxlinks": 200,
        "maxnclouds": 2000,
        "duration_range": [2, 30],
        ...
        "mcs_pf_durationthresh": float(
            max(0.0, structural_duration_hours - time_resolution_hours)
        ),
...
def prepare_and_run_pyflextrkr(
    ...
    *,
    ...
    ir_field_name: str = "model brightness temperature",
) -> PyFLEXTRKRResult:
    source_model = str(source_model).upper()
    if ir_required:
    ...
    if len(fhrs) > 1:
        intervals = np.diff(np.asarray(fhrs, dtype=float))
        time_resolution_hours = float(np.median(intervals))
        if not np.allclose(intervals, time_resolution_hours):
            raise ValueError(
                "PyFLEXTRKR input forecast hours must have a fixed interval; "
                f"received {fhrs}"
            )
    else:
